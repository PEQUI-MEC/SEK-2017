...
void Resgate::resgatar() {
	...
	while(true) {
		...
		if(carga_bonecos >= capacidade_bonecos) pwm_busca = 70;
		else pwm_busca = 40;
		...
		switch(estd) {
			case estados_arena::leu_fora:
				...
				} else if(qnt_cruzamentos >=total_cruzamentos_teste && carga_bonecos >= capacidade_bonecos) {
					//saindo da ultima intersec, subir rampa
			...
			case estados_arena::intersec:
				...
				if(qnt_cruzamentos<= 0) {
					...
					estd = estados_arena::faixa;
					cout << "FAIXA!!" << endl;
					if(carga_bonecos <= capacidade_bonecos) robo->andar(60, 0.2); 
				}
			...
			case estados_arena::faixa:
				...
				if(carga_bonecos >= capacidade_bonecos) robo->andar(70);
				else robo->andar(pwm_busca);
				
				if (cor_E != Cor::branco || cor_D != Cor::branco) {
					if(qnt_cruzamentos >=total_cruzamentos_teste && sentido_navegacao == 1) {
						//saindo da ultima intersec, subir rampa
			...
			case estados_arena::captura: // caso ver algum boneco
				captura_rogerio();
				pwm_busca = 40;
				// o estado é atribuido dentro do metodo de captura
...
void Resgate::intersec() {
	if(sentido_navegacao == -1) qnt_cruzamentos --;
	else qnt_cruzamentos ++;
...
void Resgate::go_to_plaza2() {
	...
	while(true) {
		if((cor_E == Cor::vermelho && cor_D == Cor::vermelho) || (cor_E == Cor::verde && cor_D == Cor::verde))
			break;
	}
