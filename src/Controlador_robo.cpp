	...
	while (thread_rodando) {
		...
		switch(estado) {
			...
			case flag_aceleracao::linha_reta :
				distancia_linha_reta = -(motorE->position()+motorD->position())/2 *3.141592/180*relacao_engrenagem*raio_roda;
				...
				*/
				...
				if(pwm_retardada < pwm_sp-aceleracao*delay/1000) pwm_retardada += aceleracao*delay/1000;
				if(pwm_retardada > pwm_sp+aceleracao*delay/1000) pwm_retardada -= aceleracao*delay/1000;
...
void Controlador_robo::calibra_sensor_cor(Sensor_cor_hsv *sensor_cor) {
	...
	while(!ev3dev::button::enter.process()) {
		...
		if(max_rgb_E[0] < get<0>(sample_E)) max_rgb_E[0] = get<0>(sample_E);
